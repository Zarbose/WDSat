...
inline int_t dimacs_get_set_vars(){return nb_var;};

inline int_t dimacs_increase_set_vars(){nb_var++;};
inline int_t dimacs_decrease_set_vars(){nb_var--;};
/* ----------------- */
...
void dimacs_print_equivalency() {
    ...
    for(i = 0LL; i < dimacs_nb_of_boolean_equations; ++i) {
        if (dimacs_size_of_boolean_equation[i] == 3){
            printf(" %lld <=> %lld %lld",dimacs_boolean_equation[i][2LL],-dimacs_boolean_equation[i][0LL],-dimacs_boolean_equation[i][1LL]);
        }
        else{
            continue;
        }
